...
- **📅/⚠️ 블록쿼트 위치**: 본문 최상단 ❌, 도입부 첫 문단 다음도 ❌(도입부가 끊김) →
  **도입부 전체 다음, 첫 H2 직전** (홈 발췌문 보일러플레이트 방지, 2026-08-08 재수정)
...
### blog-writer 의무 사항
**도입부 전체를 다 쓴 다음, 첫 H2 직전에** 업데이트 날짜·주의 문구를, 본문 최하단에 공식 출처 표를 반드시 삽입한다:
...
> ⚠️ **최상단에 넣지 말 것 (2026-08-06 변경, 2026-08-08 위치 재수정).** Tistory 홈 목록
> 발췌문이 본문 맨 앞을 가져가므로, 최상단에 두면 홈 글 목록 8개가 전부 같은 문구로 시작한다 —
> 애드센스 심사관이 보는 첫 화면이 보일러플레이트 반복이 되어 대량 생성 사이트로 읽힌다.
> 단 **"도입부 첫 문단 다음"은 아니다** — 도입부가 여러 문단이면 한 문장만 지나 공지가
> 끼어들어 나머지 도입부가 뒤로 밀리는 부자연스러운 결과가 났다(실제 발행글에서 확인).
> 도입부를 끝까지 하나의 흐름으로 쓰고, 그 다음(첫 H2 직전)에 넣는다.
