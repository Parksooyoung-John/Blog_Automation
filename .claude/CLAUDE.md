...
## 커밋 규칙 — 재발 방지를 같은 커밋에 넣는다

**사용자가 "커밋하자"고 하면 코드만 담지 않는다.** 그 세션에서 실제로 발생한 문제에 대해
구조적 재발 방지까지 같은 커밋에 포함한다. 사용자가 따로 요청하지 않아도 한다.

### 커밋 전 점검 (매번)

1. **이번 세션에서 무엇이 어긋났는지 적는다** — 버그, 잘못된 보고, 조용히 깨져 있던 검증,
   틀린 안내, 사용자가 두 번 지적한 것. "고쳤으니 됐다"로 넘기지 않는다.
2. 각 항목에 대해 **"같은 일이 또 생기면 무엇이 먼저 알려주나?"**를 답한다.
   답이 "아무것도 없다"면 그 자리에 장치를 만든다.
3. 장치를 만들고 **실행해서 통과를 확인한 뒤** 커밋한다.

### 방지 수단의 우선순위

높은 것부터 시도한다. **문서만 추가하는 것은 최후 수단이다** — 문서는 안 읽히면 아무것도 막지 못한다.

| 순위 | 수단 | 예 |
|---|---|---|
| 1 | 코드가 스스로 깨지게 | `--selftest`에 실제 표기법을 합성 입력으로 고정, 함수 진입부 `assert` |
| 2 | 검사 스크립트에 항목 추가 | `verify_naver.py`의 카테고리 일치·발행 간격·태그 중복 |
| 3 | 실패 시 종료 코드 | 게이트가 통과처럼 보이지 않게 `sys.exit(1)` |
| 4 | 문서 규칙 | 위 셋으로 표현할 수 없는 판단·운영 규칙만 |

### 커밋 메시지

**무엇을 고쳤는지가 아니라 무엇이 왜 깨졌는지를 쓴다.** 나중에 같은 패턴을 알아보기 위해서다.
숫자가 틀렸으면 틀린 값과 맞는 값을 같이 적는다(예: 1,505자를 1,728자로 보고했다).

### 같은 패턴이 두 번 이상이면

`파이프라인 기술 노트`의 재발 이력 표에 행을 추가한다. 세 번째부터는 개별 수정이 아니라
**패턴 자체를 막는 장치**를 만든다(셀프테스트 고정이 그 예다).

### 범위 가드

- **이번 세션에서 실제로 발생한 문제만** 다룬다. 있을 법한 문제를 가정해 미리 만들지 않는다.
- 재발 방지가 커져서 별도 작업이 되면, 커밋을 나누고 사용자에게 무엇을 분리했는지 말한다.
- 사용자가 지적하지 않은 기존 코드를 이 명목으로 리팩터링하지 않는다.
...
> ⚠️ **이 함정은 세 번 반복됐다.** 규칙 파일에 **완성된 한글 문장을 코드블록에 넣으면**
> 에이전트는 변수 부분만 채우고 문장은 통째로 복사한다. 실측 확산률(2026-08-14, 라이브 113개):
> `## 마무리` 65개(58%) · `개인적으로는` 64개(57%) · `이런 분께 해당합니다` 50개(44%).
> **새 규칙을 쓸 때 예시 문장을 넣지 말고 "무엇을 담아야 하는가"만 규정할 것.**
> 예시가 꼭 필요하면 ❌/✅ 대조 형태로 쓰고 "형식만 참고, 문장 복사 금지"를 명시한다.
...
- **적용 대상 분기(규칙 13)**: `이런 분께 해당합니다`·`이런 분은 건너뛰셔도 됩니다`
  **고정 문구 폐지(2026-08-14)** → 기능만 규정, 문구·형식은 글마다 새로 쓸 것
- **작성자 의견(규칙 3)**: `개인적으로는` 문두 금지 → 문장 구조 자체를 매번 다르게
...
### [verify_post.py] 카드 이미지 검증이 몇 주간 완전히 무력화돼 있었음 (2026-08-22 발견)

**문제**: `verify_post.py`의 카드 이미지 검증이 `"thumb/R800x0" in u`(구 daumcdn 프록시 URL 패턴)만
필터로 썼는데, 2026-08-03 `thumb_host.py` 도입 이후 카드 이미지는 전부 jsdelivr CDN URL
(`cdn.jsdelivr.net/.../assets/thumbnails/*.jpg`)로 바뀌었다. 즉 **패턴에 안 걸려 `imgs`가 항상
빈 리스트가 되고, 그 사이 발행된 모든 글에서 카드 이미지 검증이 조용히 통과 처리되고 있었다.**
CLAUDE.md에는 "썸네일 URL 만료 — 근본 해결됨"이라고 기록해뒀지만, 코드 쪽은 옛 패턴에 맞춰진 채
방치돼 문서와 실제 동작이 어긋나 있었다.

**해결**: 필터를 `"thumb/R800x0" in u or ("cdn.jsdelivr.net" in u and "thumbnails" in u)`로 확장해
두 패턴 모두 인식하도록 수정. 수정 직후 라이브 6개 포스트로 재검증해 실제로 카드 이미지를 다시
검사한다는 것을 확인함.

**재발 방지 원칙**: CDN·URL 구조를 바꾸는 마이그레이션(썸네일 호스팅 방식 전환 등)을 할 때는
**그 URL 패턴에 의존하는 검증 스크립트도 함께 확인**할 것 — 마이그레이션 자체는 성공해도, 그
성공을 검증하던 도구가 옛 패턴에 멈춰 있으면 "검증 통과"가 "검증이 아예 실행 안 됨"으로 조용히
바뀐다. 이런 사각지대는 에러를 내지 않으므로 별도로 찾아보지 않으면 영원히 못 알아챈다.

---

### [verify_post.py] 슬러그 특수문자 목록에 `%` 추가

**문제**: 제목에 쉼표·물음표뿐 아니라 `%`(퍼센트) 기호가 있어도 Tistory가 슬러그 생성 시 해당
문자를 제거한다("전세대출 갱신, 대출금 10% 안 갚으면 금리가 오른다" → 발행 패널이 보여준
슬러그 URL은 그대로였지만 실제 라이브 슬러그에서는 `%`가 빠짐). `_strip_slug_punctuation()`이
쉼표·물음표만 제거해서 자동 복구에 실패, `verify_post.py`가 "HTTP 404 (대체 URL도 404)"로
잘못 보고했다(실제로는 숫자 URL은 200 정상).

**해결**: `_strip_slug_punctuation()`의 정규식을 `[,?]` → `[,?%]`로 확장.

**재발 방지**: 슬러그 불일치는 제목에 URL 예약문자·특수문자가 들어갈 때마다 새로 생길 수 있는
문제다. 앞으로 비슷한 패턴(HTTP 404 + 대체 URL도 404)이 다시 나오면, 먼저 홈페이지에서 최신
글의 숫자 ID를 직접 확인해 실제로 발행이 실패했는지부터 확인할 것 — 슬러그만 깨졌을 뿐 숫자
URL은 정상인 경우가 많다.

---

### [03_tistory_playwright.py] Notion "처리중" 좀비 상태 — 타임아웃 시 재시도 루프가 못 찾음 (2026-08-22)

**문제**: `fetch_pending_posts()`가 "발행대기" 상태만 조회한다. 발행 스크립트가 도구 타임아웃
(Bash 300초 등)으로 강제 종료되면, `process_all()`이 이미 상태를 "처리중"으로 바꿔놓은 뒤라서
`except` 블록(→"오류" 전환)조차 못 타고 죽는다. 결과적으로 그 항목은 "처리중"에 영원히 멈춰
있고, 재시도해도 `fetch_pending_posts()`가 "발행대기"만 찾으므로 "발행 대기 중인 포스팅 없음"만
반복 출력하며 그 포스트를 영영 재시도하지 못한다.

**해결**: `fetch_pending_posts()` 호출 시작 시 `_recover_stuck_processing()`을 먼저 실행해
"처리중" 상태 항목을 자동으로 "발행대기"로 되돌린다. 이 파이프라인은 한 번에 한 인스턴스만
순차 실행하는 전제이므로, 스크립트 시작 시점에 "처리중"이 남아있다는 것 자체가 이전 실행이
비정상 종료됐다는 신호로 봐도 안전하다.
...
def fetch_pending_posts() -> list:
    _recover_stuck_processing()   # ← 조회 전에 좀비 상태 복구
    ...
```

**재발 방지**: 앞으로 발행 스크립트가 "처리중"에서 멈춘 걸 발견하면(홈에 최신글이 안 보이는데
Notion은 "처리중"인 경우), 수동으로 Notion API PATCH를 날릴 필요 없이 그냥 스크립트를 한 번 더
실행하면 자동 복구된다.

---

### [검수 스크립트] 표기·구조를 바꾸면 검증 도구가 조용히 무력화된다 — 세 번째 재발 (2026-09-26)

**문제**: 네이버 원고의 플레이스홀더가 `[이미지: ...]` → `[이미지 ①: ...]`로 바뀌었는데
`check_naver.py`의 정규식이 콜론을 고정(`\[이미지:`)하고 있었다. 이미지·경험을 **0으로** 세고,
분량에서도 플레이스홀더를 못 걸러 1,505자를 1,728자로 보고했다. **에러는 안 나고 숫자만 틀렸다.**

**같은 패턴이 세 번째다.**

| 시점 | 무엇을 바꿨나 | 무엇이 조용히 깨졌나 |
|---|---|---|
| 2026-07-17 | blog-writer 메타 필드 5개 | `parse_blog_post()` 블록쿼트 화이트리스트 |
...
셋 다 **검증 대상의 표기·구조를 바꾸고 그걸 검증하던 스크립트를 같이 안 고쳤고**,
셋 다 **에러 없이 통과처럼 보였다.** 따로 찾아보지 않으면 영원히 못 알아챈다.

**해결 — 검수 스크립트에 `--selftest`를 고정한다.** 네이버 계열 셋은 전부 갖췄다.
...
**규칙**: 원고 표기·HTML 구조·API 필드에 의존하는 정규식을 손대면 **같은 커밋에서**
셀프테스트를 갱신하고 실행할 것. 티스토리 쪽은 `04_notion_upload.py --selftest`가 같은 역할이다.

---

### [verify_naver.py] 필드 하나만 보고 "없다"고 말하지 않는다 (2026-09-26)

1편 점검에서 `baLogData`의 `"tagNames":""`를 보고 "태그가 하나도 없습니다"라고 보고했다.
실제 태그 필드는 `var gsTagName = "..."`이었고 태그 10개가 정상적으로 들어가 있었다.
사용자가 멀쩡한 글을 수정하러 들어갈 뻔했다.

**규칙**: API·HTML에서 **필드 하나만 보고 "없다"고 단정하지 않는다.** 본문에서 실제 값을
교차 확인하고(예: 태그 문자열을 HTML 전체에서 grep), 확인이 안 되면 "이 필드에서는 안 보인다"로
범위를 좁혀 보고한다. 발행된 글을 고치라는 지시는 되돌릴 수 없는 작업이다.
