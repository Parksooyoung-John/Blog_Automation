...
- **📅/⚠️ 블록쿼트 위치**: 본문 최상단 ❌ → **도입부 첫 문단 다음** (홈 발췌문 보일러플레이트 방지)
...
### blog-writer 의무 사항
**도입부 첫 문단 다음에** 업데이트 날짜·주의 문구를, 본문 최하단에 공식 출처 표를 반드시 삽입한다:
...
> ⚠️ **최상단에 넣지 말 것 (2026-08-06 변경).** Tistory 홈 목록 발췌문이 본문 맨 앞을 가져가므로,
> 최상단에 두면 홈 글 목록 8개가 전부 같은 문구로 시작한다 — 애드센스 심사관이 보는 첫 화면이
> 보일러플레이트 반복이 되어 대량 생성 사이트로 읽힌다. ⚠️ 문구도 매번 같게 복사하지 말 것.
