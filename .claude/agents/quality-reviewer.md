...
### 블로그 벤치마킹 패턴 검증
- [ ] 제목 앞부분에 핵심 검색 키워드가 배치됐는가 — 없으면 🟡 권장 수정
- [ ] 첫 문단에 "왜 지금 이 주제가 중요한지" 배경 설명이 있는가 — 없으면 🟡 권장 수정
- [ ] 체크리스트(`- [ ]` 형식)가 1개 이상 포함됐는가 — 없으면 🟡 권장 수정
- [ ] 금지 표현(`급등`, `폭등`, `대박`, `무조건`, `수익 보장`, `지금 사야`, `지금 바로 투자`)이 없는가 — 있으면 🔴 필수 수정
- [ ] 특정 금융상품 매수·가입 추천처럼 보이는 표현이 없는가 — 있으면 🔴 필수 수정

### 블로그 탈AI 패턴 검증
- [ ] 서론 첫 문단이 구어체("~거든요", "~더라고요")로 시작하는가 — "~입니다" 단독 시작 🟡 권장 수정
- [ ] 본문 중간에 개인 경험 또는 주변 사례 문단이 1회 이상 포함되어 있는가 — 없으면 🟡 권장 수정
- [ ] 결론 마지막 문장에 작성자 의견이 포함되어 있는가 — 없으면 🟡 권장 수정
- [ ] 금지 표현("이러한", "따라서", "이에 따라", "이를 통해", "본 글에서는")이 반복 사용되지 않는가 — 3회 이상 🟡 권장 수정
- [ ] 문장 길이가 단문·장문 혼합으로 구성되어 있는가 — 같은 길이 4문장 이상 연속 🟡 권장 수정
