...
1. 사용자 입력에서 추출한다:
   ...
   - **쿠팡파트너스 링크** (선택): `link.coupang.com` 포함 URL — 감지 시 `00_input.md`에 저장
2. `_workspace/` 디렉토리를 프로젝트 루트에 생성한다
3. 입력을 정리하여 `_workspace/00_input.md`에 저장한다

   > **쿠팡파트너스 링크가 있는 경우** `00_input.md` 맨 끝에 다음 섹션을 추가한다:
   > ```markdown
   > ## 쿠팡파트너스
   > - url: https://link.coupang.com/a/XXXXX
   > - 상품명: [사용자가 제공한 상품명, 없으면 "상품"]
   > ```
   > 감지 패턴: URL에 `link.coupang.com` 포함, 또는 사용자가 "쿠팡파트너스 링크"라고 명시.
...
### Phase 4: 포스트 인덱스 업데이트 (Tistory 발행 완료 후)

Tistory 발행이 완료되어 실제 URL이 확정되면 다음 **2단계를 순서대로** 수행한다:

**Step 4-1: OG 이미지 URL 수집 (필수)**

발행된 포스트 URL에서 OG 이미지 URL을 가져와 `_posts_index.md`의 `thumb` 필드에 기록한다. `pending`으로 남겨두면 다음 포스트 작성 시 이 포스트를 내부 링크 카드로 사용할 수 없다.

```python
# 발행 직후 실행 — 아래 패턴으로 OG 이미지 URL 추출
import requests, re
...
thumb_url = m.group(1) if m else "pending"
```

**Step 4-2: 인덱스에 새 행 추가**

```
| [포스트 제목] | https://j2gblog.tistory.com/[번호] | [카테고리] | [주요 키워드 5개] |
```

- 카테고리: `03_tistory_playwright.py` 로그에서 확인한 Tistory 카테고리 사용
- 키워드: 블로그 포스트 메타 키워드 + 본문 핵심 단어
- `thumb:` 필드: Step 4-1에서 추출한 실제 CDN URL — OG URL을 가져오지 못한 경우에만 `pending` 임시 사용
- **이 단계를 생략하면 다음 포스트에서 내부 링크가 빠짐** — 반드시 수행할 것
- **`thumb: pending`이 남아있는 상태에서 다음 배치 작업을 시작하지 말 것**
